...
class SARCBert(nn.Module):
    def __init__(self, bert_model):
        ...
        self.ancestor_layer = nn.Linear(in_features=MAX_ANCESTORS * self.bert.config.hidden_size, out_features=self.bert.config.hidden_size)
    ...
    def forward(self, input_ids, attention_mask, ancestor_input_ids,
        ...
        ancestor_cls_tokens = F.relu(self.ancestor_layer(ancestor_cls_tokens))
...
class SARCBertDataCollator:
    ...
    def __call__(self, features):
        ...
        padded_ancestor_ids['input_ids'] = torch.tensor(padded_ancestor_ids['input_ids']).reshape(MAX_ANCESTORS, len(ancestor_input_ids), -1).tolist()
        padded_ancestor_ids['attention_mask'] = torch.tensor(padded_ancestor_ids['attention_mask']).reshape(MAX_ANCESTORS, len(ancestor_input_ids), -1).tolist()
